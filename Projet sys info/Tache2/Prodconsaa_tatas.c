...
void *producteur(){
  ...
  while(true)
  {
    ...
    item=produce(item);
    my_sem_wait_tatas(&empty); // attente d'une place libre
    my_lock_tatas(&mutex);
    ...
    my_unlock(&mutex);
    my_sem_post_tatas(&full); // il y a une place remplie en plus
  }
  return NULL;
}
...
void *consumer()
{
 ...
 while(true)
 {
    ...
    my_sem_wait_tatas(&full); // attente d'une place remplie
    my_lock_tatas(&mutex);
    ...
    my_unlock(&mutex);
    my_sem_post_tatas(&empty); // il y a une place libre en plus
    consume(item);
 }
 return NULL;
}
